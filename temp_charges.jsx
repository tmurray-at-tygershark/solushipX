...
const ChargesTab = () => {
    ...
    const handleStartEditInvoiceStatus = (chargeId, currentStatus) => {
        setEditingInvoiceStatus(chargeId);
        setEditingStatusValue(currentStatus);
    };
    ...
    const handleSaveInvoiceStatus = async (chargeId) => {
        if (!editingStatusValue || editingStatusValue === charges.find(c => c.id === chargeId)?.status) {
        ...
        try {
            ...
            // Update the shipment's invoice status
            const shipmentRef = doc(db, 'shipments', charge.shipmentID);

            // Check if the document exists before updating
            const shipmentDoc = await getDoc(shipmentRef);
            if (!shipmentDoc.exists()) {
                // If document doesn't exist, create it with the status
                await setDoc(shipmentRef, {
                    status: editingStatusValue,
                    updatedAt: new Date(),
                    updatedBy: currentUser.email,
                    shipmentID: charge.shipmentID,
                    // Add other basic fields that might be needed
                    createdAt: new Date(),
                                    ...
                                    charges.map((charge) => (
                                        <React.Fragment key={charge.id}>
                                            ...
                                            >
                                                ...
                                                <TableCell sx={{ fontSize: '12px', verticalAlign: 'top' }}>
                                                    {editingInvoiceStatus === charge.id ? (
                                                        // Inline editing mode with styled status options
                                                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                                            {/* Current selection display */}
                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                                {(() => {
                                                                    const dynamicStatus = invoiceStatuses.find(s => s.statusCode === editingStatusValue);
                                                                    return (
                                                                        <Chip
                                                                            label={dynamicStatus?.statusLabel || editingStatusValue}
                                                                            size="small"
                                                                            sx={{
                                                                                fontSize: '11px',
                                                                                height: '20px',
                                                                ...
                                                                <IconButton
                                                                    size="small"
                                                                    onClick={handleCancelEditInvoiceStatus}
                                                                ...
                                                                <IconButton
                                                                    size="small"
                                                                    onClick={() => handleSaveInvoiceStatus(charge.id)}
                                                                    disabled={savingStatus}
                                                                    sx={{
                                                                        color: '#059669',
                                                                        p: 0.5,
                                                            ...
                                                            <Box sx={{
                                                                ...
                                                                {invoiceStatuses.filter(status => status.enabled).map((status) => (
                                                                    <Chip
                                                                        ...
                                                                        label={status.statusLabel}
                                                                        size="small"
                                                                        onClick={() => setEditingStatusValue(status.statusCode)}
                                                                        sx={{
                                                                            fontSize: '10px',
                                                                            height: '18px',
                                                                            fontWeight: 500,
                                                                            ...
                                                                            border: '1px solid rgba(0,0,0,0.1)',
                                                                            cursor: 'pointer',
                                                                            opacity: editingStatusValue === status.statusCode ? 1 : 0.7,
                                                                            '&:hover': {
                                                                                opacity: 1,
                                                        ...
                                                        // Display mode with status chip and dropdown arrow
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                            {(() => {
                                                                ...
                                                                return (
                                                                    <React.Fragment>
                                                                        <Chip
                                                                            label={dynamicStatus ? dynamicStatus.statusLabel : statusCode}
                                                                            size="small"
                                                                            sx={{
                                                                                fontSize: '11px',
                                                                                height: '20px',
                                                                                fontWeight: 500,
                                                                                color: dynamicStatus ? (dynamicStatus.fontColor || '#ffffff') : '#ffffff',
                                                                                backgroundColor: dynamicStatus ? (dynamicStatus.color || '#6b7280') : '#6b7280',
                                                                                border: '1px solid rgba(0,0,0,0.1)',
                                                                                cursor: 'default'
                                                                            }}
                                                                        />
                                                                        <IconButton
                                                                            size="small"
                                                                            onClick={() => handleStartEditInvoiceStatus(charge.id, charge.status || 'uninvoiced')}
                                                                            sx={{
                                                                                p: 0.5,
                                                                                color: '#6b7280',
                                                                                '&:hover': {
