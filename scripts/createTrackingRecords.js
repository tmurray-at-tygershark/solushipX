...
function generateTrackingEvents(carrier, startDate) {
  const events = [];
  const statuses = [
    { status: 'picked_up', description: 'Package picked up' },
...
async function createTrackingRecords() {
  try {
    console.log('Starting tracking records creation...');
    ...
    for (const doc of shipmentsSnapshot.docs) {
      ...
      // Add tracking record
      await addDoc(collection(db, 'tracking'), trackingData);
      console.log(`Created tracking record for shipment ${shipment.shipmentId}`);
    }
    
    console.log('Successfully created tracking records for all shipments');
