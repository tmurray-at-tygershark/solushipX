...
async function recreateShipmentsCollection() {
  try {
    console.log('Starting shipment collection recreation process...');
    ...
    // Keep track of shipment counts per customer
    ...
    for (let i = 0; i < 20; i++) {
      ...
      // Increment the shipment count for this customer
      customerShipmentCounts[customerId] = (customerShipmentCounts[customerId] || 0) + 1;
      const shipmentNumber = customerShipmentCounts[customerId];
      
      // Create the shipmentID
      const shipmentId = `${companyId}-${customerId}-${shipmentNumber}`;
      ...
      const shipmentData = {
        ...
        // Packages
        packages: Array(packageCount).fill(null).map(() => generateRandomPackage()),
      ...
      await addDoc(shipmentsRef, shipmentData);
      
      console.log(`Created shipment ${shipmentId} with ${packageCount} packages`);
    }
    
    console.log('Successfully recreated shipments collection with 20 new shipments');
    
  } catch (error) {
    console.error('Error recreating shipments collection:', error);
  } finally {
    process.exit(0);
  }
}
