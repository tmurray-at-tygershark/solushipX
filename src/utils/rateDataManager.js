    ...
    if (isQuickShip) {
        ...
        if (shipment.manualRates && Array.isArray(shipment.manualRates)) {
            charges = shipment.manualRates.map(rate => ({
                ...
                // PRESERVE SEPARATE ACTUAL AND QUOTED VALUES FOR QUICKSHIP - NO FALLBACKS TO PREVENT DUPLICATION
                quotedCost: parseFloat(rate.quotedCost || rate.cost || rate.charge || 0),
                quotedCharge: parseFloat(rate.quotedCharge || rate.charge || rate.cost || 0),
        ...
        if (shipment.updatedCharges && Array.isArray(shipment.updatedCharges)) {
            charges = shipment.updatedCharges.map(charge => ({
                ...
                // PRESERVE SEPARATE ACTUAL AND QUOTED VALUES
                quotedCost: parseFloat(charge.quotedCost || charge.cost || 0),
                quotedCharge: parseFloat(charge.quotedCharge || charge.charge || 0),
                actualCost: parseFloat(charge.actualCost || charge.cost || 0),
                actualCharge: parseFloat(charge.actualCharge || charge.charge || 0),
    ...
    try {
        ...
        if (isQuickShip) {
            ...
            updateData.updatedCharges = rateData.charges.map(charge => ({
                ...
                quotedCost: charge.quotedCost || charge.cost,
                quotedCharge: charge.quotedCharge || charge.charge,
                actualCost: charge.actualCost || charge.cost,
                actualCharge: charge.actualCharge || charge.charge,
