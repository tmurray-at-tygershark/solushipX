    ...
    if (isQuickShip) {
        ...
        if (shipment.manualRates && Array.isArray(shipment.manualRates)) {
            charges = shipment.manualRates.map(rate => ({
                ...
                // Use nullish checks so 0 is respected and not treated as missing
                cost: rate.cost != null ? parseFloat(rate.cost) : (rate.charge != null ? parseFloat(rate.charge) : 0),
                charge: rate.charge != null ? parseFloat(rate.charge) : (rate.cost != null ? parseFloat(rate.cost) : 0),
                // Preserve separate values and respect zero
                quotedCost: rate.quotedCost != null ? parseFloat(rate.quotedCost) : (rate.cost != null ? parseFloat(rate.cost) : (rate.charge != null ? parseFloat(rate.charge) : 0)),
                quotedCharge: rate.quotedCharge != null ? parseFloat(rate.quotedCharge) : (rate.charge != null ? parseFloat(rate.charge) : (rate.cost != null ? parseFloat(rate.cost) : 0)),
        ...
        if (shipment.updatedCharges && Array.isArray(shipment.updatedCharges)) {
            charges = shipment.updatedCharges.map(charge => ({
                ...
                // Respect zero values using nullish checks
                cost: charge.actualCost != null ? parseFloat(charge.actualCost) : (charge.quotedCost != null ? parseFloat(charge.quotedCost) : (charge.cost != null ? parseFloat(charge.cost) : 0)),
                charge: charge.actualCharge != null ? parseFloat(charge.actualCharge) : (charge.quotedCharge != null ? parseFloat(charge.quotedCharge) : (charge.charge != null ? parseFloat(charge.charge) : 0)),
                // Preserve separate ACTUAL and QUOTED values, honoring zeros
                quotedCost: charge.quotedCost != null ? parseFloat(charge.quotedCost) : (charge.cost != null ? parseFloat(charge.cost) : 0),
                quotedCharge: charge.quotedCharge != null ? parseFloat(charge.quotedCharge) : (charge.charge != null ? parseFloat(charge.charge) : 0),
    ...
    try {
        ...
        if (isQuickShip) {
            ...
            updateData.updatedCharges = rateData.charges.map(charge => ({
                ...
                quotedCost: charge.quotedCost != null ? charge.quotedCost : (charge.cost != null ? charge.cost : 0),
                quotedCharge: charge.quotedCharge != null ? charge.quotedCharge : (charge.charge != null ? charge.charge : 0),
                actualCost: charge.actualCost != null ? charge.actualCost : (charge.cost != null ? charge.cost : 0),
                actualCharge: charge.actualCharge != null ? charge.actualCharge : (charge.charge != null ? charge.charge : 0),
