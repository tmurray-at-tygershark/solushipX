...
const EditCustomer = ({ customerId = null, customerFirestoreId = null, onBackToDetail = null, onCustomerUpdated = null, onClose = null, isModal = false }) => {
    const { id: urlCustomerFirestoreId } = useParams();
    const navigate = useNavigate();
    const { enqueueSnackbar } = useSnackbar();
    const { user } = useAuth();
    const { companyIdForAddress, companyData } = useCompany();

    // Use prop customerId if provided, otherwise fall back to URL parameter or prop
    const effectiveCustomerId = customerId || customerFirestoreId || urlCustomerFirestoreId;
    const isEditMode = Boolean(effectiveCustomerId);
    ...
    const loadCustomerData = useCallback(async () => {
        ...
        try {
            ...
            console.log('[EditCustomer] Trying to fetch customer by document ID:', effectiveCustomerId);
            const customerDocRef = doc(db, 'customers', effectiveCustomerId);
            const customerDoc = await getDoc(customerDocRef);

            if (customerDoc.exists()) {
                customerData = customerDoc.data();
                actualDocumentId = customerDoc.id;
                console.log('[EditCustomer] Customer found by document ID:', customerData);
            } else {
                // If not found by document ID, try to find by customerID field
                console.log('[EditCustomer] Customer not found by document ID, trying customerID field:', effectiveCustomerId);
                ...
                if (!customerSnapshot.empty) {
                    ...
                    console.log('[EditCustomer] Customer found by customerID field:', customerData);
                } else {
            ...
            if (customerData && (!customerData.mainContactName || !customerData.mainContactAddress1)) {
                console.log('[EditCustomer] No contact info in customer document, checking addressBook...');
                ...
                if (customerData.customerID) {
                    try {
                        ...
                        if (!mainContactSnapshot.empty) {
                            ...
                            console.log('[EditCustomer] Found contact in addressBook:', contactData);
                            ...
                            console.log('[EditCustomer] No contact found in addressBook for customerID:', customerData.customerID);
                        }
                    } catch (addressError) {
                        console.error('[EditCustomer] Error loading contact from addressBook:', addressError);
                    }
                }
            }
            ...
            setFormData({
                ...
                companyID: customerData.companyID || companyIdForAddress || '',
    ...
    }, [isEditMode, effectiveCustomerId, companyIdForAddress]);
    ...
    useEffect(() => {
        if (isEditMode) {
            ...
            // Set default company for new customer
            setFormData(prev => ({
                ...prev,
                companyID: companyIdForAddress || ''
            }));
    ...
    }, [loadCustomerData, isEditMode, companyIdForAddress]);

    // Auto-generate customer ID when name changes (only for new customers)
    useEffect(() => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEditMode) {
                ...
                await updateDoc(doc(db, 'customers', effectiveCustomerId), customerData);

                enqueueSnackbar('Customer updated successfully!', { variant: 'success' });

                if (onCustomerUpdated) {
                    onCustomerUpdated(customerData);
                }
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
                        <Box>
                            ...
                            <Typography sx={{ fontSize: '12px', color: '#64748b' }}>
                                {isEditMode ? 'Update customer information and contact details' : 'Add a new customer with contact information and address details'}
                            </Typography>
                        </Box>
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                            ...
                            >
                                {saveLoading ? 'Saving...' : (isEditMode ? 'Update Customer' : 'Create Customer')}
                            </Button>
                        </Box>
                    </Box>
                </Box>
