...
const ChargeTypeDetailDialog = ({
    ...
    // Extract and classify charges
    const chargeBreakdown = charges.length > 0 ? charges :
        (shipment.chargesBreakdown || shipment.actualCharges || []);

    const classifiedCharges = chargeTypeService.classifyCharges(chargeBreakdown);
    const categoryStats = chargeTypeService.getCategoryStatistics(classifiedCharges);
                        ...
                        <Box sx={{ mb: 3 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                {Object.entries(categoryStats).map(([categoryLabel, stats]) => {
                                    const category = Object.values(chargeTypeService.CHARGE_CATEGORIES)
                                        .find(cat => chargeTypeService.getCategoryInfo(cat).label === categoryLabel);
